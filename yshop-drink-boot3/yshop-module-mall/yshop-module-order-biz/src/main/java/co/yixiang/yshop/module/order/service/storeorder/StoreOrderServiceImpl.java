    ...
    public void orderRefund(Long id, BigDecimal price, Integer type, Long salesId) {
        ...
        if (PayTypeEnum.YUE.getValue().equals(storeOrderDO.getPayType())) {
            ...
            MerchantDetailsDO merchant = merchantDetailsService.getMerchantDetailsList(
                java.util.Collections.singletonList("yeepay")).stream().findFirst().orElse(null);
            if (merchant == null) {
                throw exception(new ErrorCode(999996, "易宝商户配置不存在"));
            }
            YeepayRefundReqVO refundReqVO = new YeepayRefundReqVO();
            refundReqVO.setMerchantNo(merchant.getMchId());
            refundReqVO.setOrderId(storeOrderDO.getOrderId());
            refundReqVO.setRefundRequestId(orderSn);
            refundReqVO.setRefundAmount(price);
            refundReqVO.setDescription("订单退款");
            refundReqVO.setMemo("订单退款");
            // 可设置notifyUrl等参数
            YeepayRefundRespVO refundRespVO = yeepayRefundService.refund(refundReqVO);
            if (!refundRespVO.isSuccess()) {
                log.error("[易宝退款] 失败: {}", refundRespVO.getMsg());
                throw exception(new ErrorCode(999999, "易宝退款失败: " + refundRespVO.getMsg()));
            }
