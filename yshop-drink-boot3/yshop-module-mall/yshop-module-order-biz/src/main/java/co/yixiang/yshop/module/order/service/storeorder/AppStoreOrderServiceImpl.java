...
public class AppStoreOrderServiceImpl extends ServiceImpl<StoreOrderMapper,StoreOrderDO> implements AppStoreOrderService {
    ...
    public Map<String, Object> pay(Long uid, AppPayParam param) {
        log.info("[支付主入口] uid={}, param={}", uid, param);
        ...
        switch (PayTypeEnum.toType(param.getPaytype())){
            case WEIXIN:
                log.info("[支付分支] paytype=WEIXIN, from={}", param.getFrom());
                if ("routine".equals(param.getFrom())) {
                    log.info("[易宝分支] 命中，参数: {}", param);
                    // 微信小程序走易宝支付
                    if (orderInfo == null) {
                        throw exception(STORE_ORDER_NOT_EXISTS);
                    }
                    YeepayOrderCreateReqVO reqVO = new YeepayOrderCreateReqVO();
                    reqVO.setOpenid(memberUserDO.getRoutineOpenid());
                    reqVO.setOrderId(orderInfo.getOrderId());
                    reqVO.setAmount(orderInfo.getPayPrice());
                    reqVO.setSubject("商品购买"); // 可根据实际业务调整
                    YeepayOrderCreateRespVO yeepayResp = yeepayOrderService.createPrePayOrder(reqVO);
                    if (!yeepayResp.isSuccess()) {
                        throw exception(new ErrorCode(999999, yeepayResp.getMsg()));
                    }
                    // 恢复为返回data.data结构
                    String prePayTn = yeepayResp.getPrePayTn();
                    Object payData = prePayTn;
                    try {
                        payData = JSON.parse(prePayTn);
                    } catch (Exception ignore) {}
                    map.put("data", payData);
                    map.put("trade_type", "JSAPI");
                    log.info("[易宝分支] 返回: {}", map);
                    break;
                }
                if(AppFromEnum.H5.getValue().equals(param.getFrom())){
                    detailsId = PayIdEnum.WX_WECHAT.getValue();
                    MerchantPayOrder h5PayOrder = new MerchantPayOrder(detailsId, "MWEB", msg,
                            msg, price, param.getUni());
                    Map<String, Object> payOrderInfo = manager.getOrderInfo(h5PayOrder);
                ...
                MerchantPayOrder jsapiPayOrder = new MerchantPayOrder(PayIdEnum.WX_WECHAT.getValue(), "JSAPI", msg,
                        msg, price, param.getUni());
                jsapiPayOrder.setOpenid(memberUserDO.getOpenid());
                map.put("data",manager.getOrderInfo(jsapiPayOrder));
